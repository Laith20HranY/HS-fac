...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (employee && employee.password === password) {
      ...
      return res.json({ redirect: '/otp.html' }); // Send JSON response
    }
...
// Endpoint to get OTP
app.get('/get-otp/:employee_id', (req, res) => {
  const employee_id = req.params.employee_id;
  const otp = generateOtp();
  otps[employee_id] = otp;
  res.json({ otp });
});
...
app.post('/verify-otp', (req, res) => {
  ...
  try {
    ...
    if (otps[employee_id] && otps[employee_id].otp === otp) {
      delete otps[employee_id]; // Clear the OTP after successful verification
      return res.json({
