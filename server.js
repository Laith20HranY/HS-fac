...
// Define the allowed IPv4 address
const allowedIP = '192.168.1.27'; // Replace with your allowed IPv4 address

// Middleware to check the client's IP address
const checkIP = (req, res, next) => {
  // Get the client's IP address using various methods
  ...
  const ipv4ClientIP = clientIP === '::1' ? '127.0.0.1' : clientIP;

  // Log detailed information for debugging
  console.log(`Client IP: ${ipv4ClientIP}`);
  console.log(`Allowed IP: ${allowedIP}`);

  // Check if the client's IP address matches the allowed IP address
  if (ipv4ClientIP === allowedIP || ipv4ClientIP === '127.0.0.1') {
    next(); // Allow the request to proceed if the IP address matches
  } else {
    console.log(`Access denied. Client IP ${ipv4ClientIP} is not allowed.`);
    res.status(403).json({ message: 'Access denied. IP address not allowed.' });
  }
};
...
app.post('/login',checkIP, async (req, res) => {
  ...
  try {
    ...
    if (employee && employee.password === password) {
      ...
      const classification = 'Worker'; // Set classification
      return res.json({ redirect: '/otp.html', classification }); // Send JSON response with classification
    }
    ...
    if (employee && employee.password === password) {
      ...
      const classification = 'Manager'; // Set classification
      return res.json({ redirect: '/otp.html', classification }); // Send JSON response with classification
    }
...
app.get('/worker-requests', async (req, res) => {
  if (req.session && req.session.workerId) {
    try {
      const requests = await Request.find({ employeeId: req.session.workerId }); // Assuming 'Request' is the model for requests
...
let managerOtp = generateOtp();
const otps = {}; // Store OTPs for each employee

function generateOtp() {
  return Math.floor(100000 + Math.random() * 900000).toString();
}

function updateManagerOtp() {
  managerOtp = generateOtp();
  console.log(`New OTP generated: ${managerOtp}`);
}

updateManagerOtp();
setInterval(updateManagerOtp, 30000);
...
app.post('/verify-otp', (req, res) => {
  ...
  console.log(`Received OTP: ${otp} for Employee ID: ${employee_id}`);
  console.log(`Stored Manager OTP: ${managerOtp}`);

  try {
    if (otp === managerOtp) {
      // Verify the classification to redirect appropriately
      if (req.session.classification === 'Worker') {
        return res.json({
          message: 'OTP verified. Login successful.',
          redirect: '/workergate.html'
        });
      } else if (req.session.classification === 'Manager') {
        return res.json({
          message: 'OTP verified. Login successful.',
          redirect: '/managergate.html'
        });
      }
    } else {
      return res.status(403).json({ message: 'Access denied. Please try again.' });
    }
  } catch (error) {
