...
app.post('/update-order', async (req, res) => {
  ...
  try {
    const updatedOrder = await Order.findOneAndUpdate(
      ...
      { machine_id, quantity, order_date, status },
      { new: true }
    );
    if (updatedOrder) {
...
app.post('/submit-dpp1', async (req, res) => {
  ...
  try {
      ...
      // Find the corresponding order in m1 and update it
      ...
      console.error('Error submitting DPP1:', error);
      res.status(500).json({ message: 'Server error, please try again.' });
  }
});

// Endpoint to handle submission of dpp2 (Daily Malfunction Report)
app.post('/submit-dpp2', async (req, res) => {
  ...
  try {
      ...
      console.error('Error submitting DPP2:', error);
      res.status(500).json({ message: 'Server error, please try again.' });
  }
});
